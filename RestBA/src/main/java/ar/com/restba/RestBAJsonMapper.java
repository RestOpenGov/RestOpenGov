 ...
 */
public class RestBAJsonMapper implements JsonMapper {
  /**
   ...
   * Logger.
   */
  private static final Logger logger = Logger.getLogger(RestBAJsonMapper.class.getName());
   ...
   * @since 1.6.2
   */
  public RestBAJsonMapper(JsonMappingErrorHandler jsonMappingErrorHandler) {
  ...
  public <T> T toJavaObject(String json, Class<T> type) {
    ...
    try {
      ...
      List<FieldWithAnnotation<RestBA>> fieldsWithAnnotation = findFieldsWithAnnotation(type, RestBA.class);
  ...
  protected void logMultipleMappingFailedForField(String facebookFieldName,
      FieldWithAnnotation<RestBA> fieldWithAnnotation, String json) {
    if (!logger.isLoggable(FINER))
      return;
   ...
   *         field.
   */
  protected String getFacebookFieldName(FieldWithAnnotation<RestBA> fieldWithAnnotation) {
   ...
   */
  protected Set<String> facebookFieldNamesWithMultipleMappings(List<FieldWithAnnotation<RestBA>> fieldsWithAnnotation) {
   ...
   *           If an error occurs while mapping JSON to Java.
   */
  protected Object toJavaType(FieldWithAnnotation<RestBA> fieldWithAnnotation, JsonObject jsonObject,
