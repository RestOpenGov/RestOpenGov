...
public class RestBAConnector extends BaseRestBAConnector implements
	...
	public RestBAConnector(String accessToken, WebRequestor webRequestor,
		...
		illegalParamNames.addAll(Arrays
				.asList(new String[] { ACCESS_TOKEN_PARAM_NAME,
						METHOD_PARAM_NAME, FORMAT_PARAM_NAME }));
	}
	...
	public <T> List<T> executeQuery(String query, Class<T> objectType,
		...
		return jsonMapper.toJavaList(
				makeRequest(
						...
						null,
						parametersWithAdditionalParameter(
								Parameter.with(QUERY_PARAM_NAME, query),
								parameters)), objectType);
	}
	...
	protected void throwFacebookResponseStatusExceptionIfNecessary(String json) {
		...
		try {
			...
			JsonObject innerErrorObject = errorObject
					.getJsonObject(ERROR_ATTRIBUTE_NAME);
	...
	/**
	 * If the {@code error} and {@code error_description} JSON fields are
	 * present, we've got a response status error for this batch API call.
	 * Extracts relevant information from the JSON and throws an exception which
	 ...
	 * exceptions.
	 * <p>
	 * Uses an instance of {@link DefaultGraphFacebookExceptionMapper} by
	 * default.
	 * 
	...
	/**
	 * A canned implementation of {@code FacebookExceptionMapper} that maps
	...
	protected String toParameterString(Parameter... parameters) {
		if (!isBlank(accessToken))
			parameters = parametersWithAdditionalParameter(
					Parameter.with(ACCESS_TOKEN_PARAM_NAME, accessToken),
					parameters);
